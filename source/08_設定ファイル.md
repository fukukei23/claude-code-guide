...
> 💡 **WSL CLI版のコンテキスト最適化プロファイル（2026-06-30）**:
> - **`cc-music` エイリアス**（`.bashrc`）: `claude --mcp-config ~/.claude/cc-music-mcp.json`。音楽/映像/画像生成タスク専用。日常MCPに minimax-official/video を追加マージ（`--strict-mcp-config` は付けない=日常MCP消失になるため）。
> - **`cc-music-mcp.json`**: minimax-official/video のみ定義（env空・秘密値なし）。
> - **日常settings.json**: minimax-official/video を mcpServers から削除（テキスト処理 `minimax` は残留）＋未使用agents 8プラグインを `enabledPlugins` で無効化。PreToolUseフックでcc側からのminimax呼出をブロック。
