...
画面下部にモデル名・コンテキスト使用率・レート制限を表示する。
...
| 表示項目 | 説明 |
|---|---|
| モデル名 | 現在のLLM（GLM-5.1等） |
| `ctx: XX%` | コンテキストウィンドウ使用率（データなし時は `---`） |
| `5h: XX%` | 5時間レート制限の使用率 |
| `7d: XX%` | 7日レート制限の使用率 |

> **注意**: コンテキスト使用率が高くなったら早めに `/compact` すること。100%に達すると `/compact` 自体も失敗し、新セッションが必要になる。
