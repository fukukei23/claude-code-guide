...
### 台帳の整合性チェックスクリプト {#check-guide-count}

公開しているガイドの「台帳」と実体が乖離していないか監視する運用スクリプトとして `scripts/config/check-guide-count.sh` を配置。

- **照合内容**: GitHub Pagesの `index.html` に掲載された自サイト内ガイドリンク数（外部URL・アンカー・`.md`除外・重複排除）と、全体マップ（SSOT）に記録した「ガイドサイト（N冊」の `N` を比較
- **カウント基準**: ディレクトリ実数ではなく **`index.html` 掲載数を正**とする（公開台帳＝`index.html` で「公開しているガイド数」と一致させるため。実数は未掲載/実体なしリンクの不整合を含む）
- **結果**: `/tmp/claude-startup/guide-count.status` に `✅`（整合）または `⚠️`（要更新・実数と乖離）を出力。SessionStartのバナー等から参照し、ズレに早く気づける
- **実行例**: `bash ~/.claude/scripts/config/check-guide-count.sh`
